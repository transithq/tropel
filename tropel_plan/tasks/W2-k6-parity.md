...
- [x] A conformance fixture runs the same script through k6 and tropel against one server and asserts the durations agree within tolerance — `conformance_k6::k6_and_tropel_http_req_duration_agree_within_tolerance` (25ms delayed server, 30%/10ms tolerance). PR #381. **Now actually runs**: the test skipped in every environment because no CI job installed k6, so the number quoted here was a one-off local run nothing could re-check. A pinned `k6-conformance` job installs k6 v1.3.0 and sets `TROPEL_REQUIRE_K6=1`, which turns a missing binary into a failure; the job is in `ci-ok`'s `needs`. ✅**MEAS** against real k6 v1.3.0: **k6 27.0405 ms** (sending 0.0074 + waiting 27.0141 + receiving 0.0190) vs **tropel 27.1318 ms** — 0.09 ms apart. The formula is genuinely right; it was simply ungated
...
- [x] **[GAP]** `ws_ping` and `ws_connecting` are not emitted; tropel emits a non-k6 `ws_req_duration` and lists `ws_session_duration` in its scaling table without emitting it — `ws_connecting` emitted (on success + failed handshake); `ws_ping`/`ws_session_duration` still open — **un-ticked**: two of the six `ws_*` metrics are still not emitted
...
- [x] `http.cookieJar()` / `new http.CookieJar()` — 4 methods; `cookiesForURL` returns **values only**; `set` parses `expires` as **RFC1123**; `clear`/`delete` work by re-setting `MaxAge=-1` — shim surface added (cookiesForURL/set/clear/delete; expires via Date); the native per-VU jar bridge wiring is a follow-up — **un-ticked**: the shim surface exists but is not connected to a jar, so `http.cookieJar()` does not affect requests
...
- [x] **[GAP]** `k6/websockets` (WHATWG `WebSocket` + `Blob`) alongside legacy `k6/ws`. **Both emit the same six `ws_*` metrics**, so metric parity doesn't tell you which API a script uses. Legacy **blocks the VU** in a callback; modern is event-loop driven. `onping`/`onpong` are k6 extensions — **implemented**: `WebSocket` + `Blob` in `deferred-modules-shim.js`, backed by the same native `__tropel_k6_ws_*` bridges as legacy `k6/ws`, with a `sleep()` event pump (tropel has no async event loop); `removeEventListener` provided (WHATWG parity, harmless)
- [x] **[GAP]** `k6/net/grpc` from scripts — note the import path is `k6/net/grpc`, and `k6/experimental/grpc` throws a "graduated" error. `Client` (`load`/`loadProtoset` are **init-only**), 17 `Status*` constants, 4 `HealthCheck*` constants including the shipped typo `HealthCheckServiceUnkown`. `invoke` default timeout **2 min** — **implemented**: `grpc.Client` (load/loadProtoset/connect/invoke/close) + all constants; `invoke` delegates to the existing `tropel-x-grpc` `GrpcProtocol` executor via a native bridge (one implementation per behaviour). **`Stream` (client.stream) not implemented** — k6's status event is registrable but never emitted; streams have no default timeout; deferred — **un-ticked**: `client.stream` is named in the criterion and is not implemented
- [x] **[GAP]** `k6/html` — `parseHTML` + 39 Selection methods. More load-bearing than it sounds: `serializeArray`/`serialize` is a real k6 login idiom — **implemented**: in-JS HTML tokenizer + full 39-method `Selection` (Add/Find/Single/Closest/Has/Not/Next/NextAll/Prev/PrevAll/Parent/Parents/Siblings/*Until/Size/End/Eq/First/Last/Contents/Text/Attr/Html/Val/Children/Each/Filter/Is/Map/Slice/Get/ToArray/Index/Data/serialize/serializeArray), replacing the regex stub; `res.html(selector)` upgraded
...
- [x] **[GAP]** `k6/experimental/{csv,fs,streams}` — **k6's own are incomplete** (no `Symbol.asyncIterator` on the CSV parser; no `pipeTo`/`pipeThrough`/`TransformStream`/`tee()` on streams), so parity here is a lower bar than the spec implies — **implemented**: `csv.parse` returns an indexable array with BOTH `Symbol.iterator` and `Symbol.asyncIterator`; `ReadableStream` (getReader/read/enqueue/close/cancel/pipeTo/pipeThrough/tee); `fs.open` returns a `File` (read/seek/stat/size/close, `fs.SeekMode`) using the `open()` bridge
...
- [x] Confine reads to the collection root, require explicit opt-in for anything outside it, and report a refusal rather than silently sending an empty body — reads are gated off entirely for untrusted collections (refusal is loud); a per-root jail is the follow-up if a legit use case needs it — **un-ticked**: the criterion asks for a root-confined read with explicit opt-in; what shipped is all-or-nothing. Safe by default, but not the criterion
