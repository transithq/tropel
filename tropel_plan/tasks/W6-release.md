...
- [ ] `cargo publish --dry-run` succeeds for every publishable crate — **NOT MET.** Verified 2026-08-29: **4 of 7 fail.** `tropel-auth`, `tropel-runtime`, `tropel-sandbox` and `tropel-native` all report `failed to select a version for the requirement 'tropel-sdk = "^0.3.0"' / candidate versions found which didn't match: 0.2.0, 0.1.0`. The tick was based on `tropel-sdk` alone passing, which it does — it is the one crate with no versioned tropel dependency. `scripts/publish-dry-run.sh` now checks all of them so the claim is re-derivable in one command, and a non-blocking `publish-dry-run` CI job reports it. This is TR-601's own stated blocker, unchanged: publishing `tropel-sdk 0.3.0` clears it, and that needs a human
...
- [ ] A release-profile build sustains 10 000 VUs without hitting the connection-pool cliff or the `MAX_WORKERS` degradation; per-VU memory measured and recorded — **reopened**: this was ticked on the shared-Runtime 57 KB figure, which was never implemented or measured (see `TR-503`). Per-VU heap is **497,584 B** ✅MEAS → ~4.6 GB of QuickJS heap at 10 000 VUs, so the sustained-10k claim needs re-running on a box that can hold it
...
- [ ] crates.io has `tropel-sdk` **0.1.0 and 0.2.0 only**; the workspace declares **0.3.0**. A `path`+`version` dep only publishes if that exact version is on the registry, so it fails for **every** dependent — **still open and still exactly this.** Two corrections to the note that closed it: `version-lockstep.sh` does **not** enforce a single version across the SDK — the SDK check is `WARN`-only and exits 0 with 0.3.0 vs 0.1.0 — and `cargo publish --dry-run` does **not** pass; it fails for the four dependents. Confirmed against crates.io: `max_version` is `0.2.0`
