...
- [x] `agent.rs:76-88` runs the **entire engine to completion** before reporting, so a distributed run is blind until it ends and a crashed agent loses everything — **fixed**: `Engine::with_snapshot_sink` streams periodic snapshots (2 s cadence) to the agent, which forwards them to the controller as progress frames; the controller reads until the final `done: true` frame
...
- [x] Retain the sink `Vec`'s capacity — `mem::take` leaves `Vec::new()`, which re-grows 4→8→16 every tick — **fixed**: `std::mem::replace(&mut batch, Vec::with_capacity(1024))` retains the capacity
...
- [ ] `tropel build` binaries hardcode `#[tokio::main(worker_threads = …)]` and can never be tuned — **verified done**: `TROPEL_TOKIO_WORKERS` env var (default 4)
