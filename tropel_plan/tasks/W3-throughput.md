...
- [x] Hash the tag set; emit protobuf; enable gzip — **all three done.** tag-set hash and gzip landed earlier; protobuf is now the DEFAULT encoding (`OtlpProtocol::Protobuf`, `Content-Type: application/x-protobuf`), with `TROPEL_OTLP_PROTOCOL=json` to opt back. Body and Content-Type both derive from the same `self.protocol`, so a format/header mismatch is structurally impossible rather than merely untested
...
- [x] A benchmark asserts the per-window CPU is under budget at 100 k samples/s — **MET, protobuf.** ✅**MEAS** (release, Apple Silicon, one real 100 ms window = 10 000 samples / 100 series, `otlp_*_encode_and_gzip_one_100ms_window`): JSON+gzip **144.25 ms** (7.2x the 20 ms budget, and above 100 ms — the output could not keep pace with its own window); protobuf+gzip **2.63 ms**, i.e. **7.6x UNDER budget** and **54.9x faster than JSON**. Wire size also drops: 4,587,245 B -> 1,998,073 B raw, 69,627 B -> 53,621 B gzipped. The original TR-304 finding is now fully answered — gzip fixed the wire, protobuf fixed the CPU
- [x] Verified against a REAL collector — ✅**EXEC** 2026-08-30: `otel/opentelemetry-collector:0.159.0` on `:4318`, fed by a live `tropel run examples/k6/k6_sample_basic.js --otlp-endpoint …` (2 VUs / 5 s / 44 requests). The collector decoded **28 metric points** — `service.name: Str(tropel)`, `InstrumentationScope tropel`, `http_reqs` / `http_req_duration` / `iterations`, with `http_reqs` arriving as **`DataType: Sum`** (correct for a Counter). **Zero** unmarshal or reject errors in the collector log. This closes the risk that the hand-rolled encoder produced bytes only our own `prost` round-trip would accept
