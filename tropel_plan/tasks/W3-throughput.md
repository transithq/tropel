...
- [ ] A 24 h soak benchmark, run in CI weekly, asserting flat memory — **REOPENED**. None of the three parts existed: no `#[ignore]` soak test (`grep -rn '#\[ignore\]' crates/` finds none), no weekly `soak` job in `ci.yml`, no RSS assertion. The artifact was `soak_memory`, a 10 000-iteration HashMap insert/remove. Deleted. A soak needs a long-running harness; criterion is the wrong tool and cannot assert
