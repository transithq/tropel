...
pub enum Shim {
    ...
    /// `Rate` / `Trend` globals — pm.js installs all of them (pm.js:1625).
    Pm,
...
fn format_shims(format: &str) -> Option<&'static [Shim]> {
    ...
    Some(match format {
        ...
        "postman" => &[DeepEqual, Pm, Chai, Lodash, CryptoJs, Exec],
        // Bruno scripts reach the same library surface plus `bru`.
        "bru" => &[DeepEqual, Pm, Chai, Lodash, CryptoJs, Exec, Bru],
        ...
        "k6" => &[DeepEqual, Pm, Chai, Lodash, CryptoJs, Exec],
        ...
        "har" | "openapi" | "http" | "insomnia" => &[DeepEqual, Pm, Exec],
        _ => return None,
    })
}
...
mod tests {
    ...
    fn postman_bundle_excludes_bru_and_keeps_the_assertion_libraries() {
        ...
        for required in ["deep-equal-shim", "pm-shim", "chai-shim", "exec-shim"] {
            assert!(
    ...
    fn script_free_formats_exclude_the_user_script_libraries() {
        ...
        for format in ["har", "openapi", "http", "insomnia"] {
            ...
            assert_eq!(
                names,
                vec!["deep-equal-shim", "pm-shim", "exec-shim"],
                "'{format}' bundle"
            );
        }
    }
